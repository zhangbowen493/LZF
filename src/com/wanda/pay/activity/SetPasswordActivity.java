...
IMVCActivity, OnClickListener {
	...
	/**
	 *  AccessType 1-设置密码 2-修改支付密码 3- 重置支付密码 4 - 重置登录密码 5-修改登录密码
	 */
	...
	private RelativeLayout activity_photo_set_top;
	private String _AcctountType;

	private RelativeLayout _SPPRL_CheckCode;
	private RelativeLayout _ACTITY_TOP_RELYOUT;
	...
	private void initData() {
		...
		_LoginName= getIntent().getStringExtra("accountNo");
	...
	public void initView() {
		...
		_ACTITY_TOP_RELYOUT=(RelativeLayout) findViewById(R.id.activity_rl_set_top_view);
		...
		activity_photo_set_top=(RelativeLayout) findViewById(R.id.activity_photo_set_top);
	...
	public void refresh(Object... param) {
		...
		if (_AccessType == 1) {
			...
			_LoginName1=getIntent().getStringExtra("loginName");
			_LEDT_Phone.setText(PhoneNumberUtil.getNumber(WDApplication.getInstance().getUserBean().getmMoblePhoneNo()));
			_ACTITY_TOP_RELYOUT.setVisibility(View.VISIBLE);
			activity_photo_set_top1.setVisibility(View.VISIBLE);
			_SPPRL_CheckCode.setVisibility(View.VISIBLE);
			activity_photo_set_top.setVisibility(View.VISIBLE);
			_SPPTV_Find.setVisibility(View.VISIBLE);
			_SPPEDT_OldPassword.setVisibility(View.VISIBLE);
			//_SPPEDT_OldPassword.setHint("请填写原支付密码");
			_operationType="2";
			tv_bottom.setText("密码由英文字母、数字或者符号组成");
		}else if(_AccessType == 3){
			_ACTWTV_Title.setText("重置支付密码");
			//			_SPPRL_CheckCode.setVisibility(View.GONE);
			...
			//			RelativeLayout.LayoutParams _ACTITY_TOP_RELYOUT=new RelativeLayout.LayoutParams
			//					(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
			//			_ACTITY_TOP_RELYOUT.setMargins(0, 0, 0, 0);
			//						_SPPEDT_OldPassword.setHint("请填写短信验证码");
			...
			if(result){
				if(Constant_data.HTTP_SUCCESS.equals(modifyResult.coder)){
					if(_AccessType ==5){
						...
						Intent intent = new Intent(mContext, UpdatePayPwdSuccessfuActivity.class);
						startActivity(intent);
						SetPasswordActivity.this.finish();
					}
				}else{
		...
		protected void onPostExecute(Boolean result) {
			...
			if(result){
				if(Constant_data.HTTP_SUCCESS.equals(loginResult.coder)){
					...
					Intent intent = new Intent(mContext, RegisteredSuccessfuActivity1.class);
	...
	//	public void refresh(Object... param) {
	//		// TODO Auto-generated method stub
	//			_LEDT_Phone.setText(_Extra_Phone);
